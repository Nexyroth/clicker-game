...
export function getCollectionBossDamageBonus() {
  return getCollectionEffectSum("bossDamageBoost");
}
export function getCollectionItemDropBonus() {
  return getCollectionEffectSum("itemDropRateBoost");
}
export function getCollectionDiamondDropBonus() {
  return getCollectionEffectSum("diamondDropBoost");
}
...
// Tự động trang bị bộ tốt nhất: xếp hạng mọi món ĐÃ SỞ HỮU theo "sức mạnh quy đổi" rồi lấy top
// EQUIP_SLOT_COUNT món. Quy đổi = baseAmount x hệ số độ hiếm, nhưng phải chuẩn hoá theo loại hiệu
// ứng vì các loại có thang đo khác nhau (critChance 0.0125 và goldenBuffDuration 2.5 giây không thể
// so trực tiếp) - nếu không chuẩn hoá thì món tăng thời lượng buff luôn thắng áp đảo 1 cách vô lý.
const EFFECT_WEIGHTS = {
  multBoost: 1,
  cpsBoost: 1,
  critDamageBoost: 0.8,
  prestigeGemBoost: 0.7,
  comboBonusBoost: 0.5,
  bossDamageBoost: 0.5,
  itemDropRateBoost: 2,
  diamondDropBoost: 2,
  critChanceBoost: 4,
  goldenBuffDurationBoost: 0.02,
};

export function autoEquipBest() {
  const owned = [...relics, ...artifacts].filter((item) => state.collection[item.id]);
  if (owned.length === 0) return { success: false, message: "Bạn chưa sở hữu Cổ Vật/Di Vật nào." };

  const scored = owned
    .map((item) => ({
      item,
      score: item.baseAmount * RARITIES[item.rarity].multiplier * (EFFECT_WEIGHTS[item.effectType] || 1),
    }))
    .sort((a, b) => b.score - a.score);

  const best = scored.slice(0, EQUIP_SLOT_COUNT).map((x) => x.item.id);
  const changed = best.join(",") !== state.equippedItems.join(",");

  state.equippedItems = best;
  updateDisplay();

  return {
    success: true,
    message: changed ? `Đã trang bị ${best.length} món mạnh nhất!` : "Bạn đã đang trang bị bộ tốt nhất rồi.",
  };
}
