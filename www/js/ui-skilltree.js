...
// Đường nối giữa 2 node liền kề - xanh nếu node phía trên đã mở (nhánh "chảy" tiếp xuống), xám nếu chưa
function renderConnector(unlocked) {
  const connector = document.createElement("div");
  connector.className = "flex justify-center";
  connector.innerHTML = `<div class="w-1 h-4 rounded-full ${unlocked ? "bg-green-400" : "bg-gray-200"}"></div>`;
  return connector;
}
...
export function renderSkillTree() {
  const branches = [
    ...
    { key: "luck", container: elements.luckBranchList },
  ];

  branches.forEach(({ key, container }) => {
    container.innerHTML = "";
    const owned = state.skillTree[key] || 0;
    skillTree[key].forEach((node, index) => {
      if (index > 0) {
        container.appendChild(renderConnector(index <= owned));
      }
