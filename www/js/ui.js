...
import { isMultiplierBuffActive, getMultiplierBuffRemainingMs, isAutoClickerBuffActive, getAutoClickerBuffRemainingMs } from "./items.js";
import { purchaseUpgrade, getUpgradeCost, getBulkUpgradeCost, getEffectiveBuyQuantity, buyQuantityMode, setBuyQuantityMode } from "./game.js";
import {
...
export function updateDisplay() {
  ...
  elements.userDiamonds.textContent = formatNumber(state.diamonds);
...
function updateEffectsIndicators() {
  ...
  const multiplierBuffOn = isMultiplierBuffActive();
  const autoClickerBuffOn = isAutoClickerBuffActive();
  if (multiplierBuffOn || autoClickerBuffOn) {
    const parts = [];
    if (multiplierBuffOn) {
      parts.push(`x${state.activeBuffs.multiplierBuff.multiplier} còn ${Math.ceil(getMultiplierBuffRemainingMs() / 1000)}s`);
    }
    if (autoClickerBuffOn) {
      parts.push(`Auto Click còn ${Math.ceil(getAutoClickerBuffRemainingMs() / 1000)}s`);
    }
    elements.itemBuffIndicator.textContent = `🧪 ${parts.join(" | ")}`;
    elements.itemBuffIndicator.classList.remove("invisible");
  } else {
    elements.itemBuffIndicator.classList.add("invisible");
  }

  // Nút click phát sáng neon khi đang có BẤT KỲ buff nào chạy - tín hiệu thị giác để người chơi
  // biết đang trong "thời gian vàng" mà tranh thủ click.
  const anyBuffActive = isGoldenBuffActive() || multiplierBuffOn || autoClickerBuffOn;
  elements.clickButton.classList.toggle("neon-pulse", anyBuffActive);
}

// Bật/tắt nút mua tuỳ theo số Nexyroth hiện có (theo đúng SỐ LƯỢNG của chế độ mua đang chọn)
export function updateUpgradeAvailability() {
  upgrades.forEach((upgrade) => {
    ...
    if (button) {
      ...
      const affordable = qty > 0 && Big.gte(state.bucks, getBulkUpgradeCost(upgrade, qty));
      if (affordable) {
...
// Nhãn nút mua theo chế độ đang chọn: "Mua" (x1), "Mua x10", "Mua xMax (x37)"...
function buyButtonLabel(upgrade) {
  if (buyQuantityMode === 1) return "Mua";
  if (buyQuantityMode === "max") {
    const qty = getEffectiveBuyQuantity(upgrade);
    return qty > 0 ? `Mua xMax (x${qty})` : "Mua xMax";
  }
  return `Mua x${buyQuantityMode}`;
}
  ...
  upgrades.forEach((upgrade) => {
    ...
    upgradeElement.innerHTML = `
            ...
            <p class="text-sm text-gray-600 mb-2">Giá: <span class="text-yellow-500">${formatNumber(
              getBulkUpgradeCost(upgrade, getEffectiveBuyQuantity(upgrade))
            )}</span> Nexyroth</p>
...
// Cập nhật lại giá tiền + nhãn nút hiển thị sau khi mua 1 nâng cấp (hoặc đổi chế độ mua hàng loạt)
export function updateUpgradeElement(upgrade) {
  ...
  upgradeElement.querySelector(
    ...
    getBulkUpgradeCost(upgrade, qty)
  )}</span> Nexyroth`;
  button.textContent = buyButtonLabel(upgrade);
}

// Đổi chế độ mua hàng loạt (x1/x10/x25/xMax) - vẽ lại giá + nhãn nút của TOÀN BỘ nâng cấp
export function updateUpgradesForNewBuyMode() {
  upgrades.forEach((upgrade) => updateUpgradeElement(upgrade));
  updateUpgradeAvailability();
  updateBuyModeButtonsHighlight();
}

function updateBuyModeButtonsHighlight() {
  const modes = [
    ["buyMode1", 1],
    ["buyMode10", 10],
    ["buyMode25", 25],
  ...
  modes.forEach(([elKey, mode]) => {
    const button = elements[elKey];
    if (!button) return;
    button.classList.toggle("bg-blue-500", buyQuantityMode === mode);
    button.classList.toggle("text-white", buyQuantityMode === mode);
    button.classList.toggle("bg-gray-200", buyQuantityMode !== mode);
    button.classList.toggle("text-gray-700", buyQuantityMode !== mode);
  });
}
...
// Hiện popup báo Nexyroth kiếm được trong lúc đi vắng, kèm % hiệu suất offline so với chơi trực tiếp
export function showOfflineEarningsModal(earnings, elapsedSeconds, efficiency) {
  ...
  elements.offlineEfficiencyNote.textContent = `(chỉ đạt ${Math.round(efficiency * 100)}% hiệu suất so với chơi trực tiếp - quay lại chơi tay để tối ưu hơn!)`;
