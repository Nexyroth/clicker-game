...
// Nhánh "luck" (Vận May) - Sát Thương Boss, Tỉ Lệ Rơi Item, Tỉ Lệ Rơi Kim Cương, Hiệu Suất Offline
export function getLuckBossDamageBonus() {
  let bonus = 1;
  getOwnedNodesWithTimes("luck").forEach(({ node, times }) => {
    if (node.type === "bossDamage") bonus *= Math.pow(1 + node.amount, times);
  });
  return bonus;
}

export function getLuckItemDropBonus() {
  let bonus = 0;
  getOwnedNodesWithTimes("luck").forEach(({ node, times }) => {
    if (node.type === "itemDrop") bonus += node.amount * times;
  });
  return bonus;
}

export function getLuckDiamondDropBonus() {
  let bonus = 0;
  getOwnedNodesWithTimes("luck").forEach(({ node, times }) => {
    if (node.type === "diamondDrop") bonus += node.amount * times;
  });
  return bonus;
}

export function getLuckOfflineEfficiencyBonus() {
  let bonus = 0;
  getOwnedNodesWithTimes("luck").forEach(({ node, times }) => {
    if (node.type === "offlineEfficiency") bonus += node.amount * times;
  });
  return bonus;
}
